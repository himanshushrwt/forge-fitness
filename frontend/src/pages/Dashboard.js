...
import { workouts, nutrition, users as usersAPI } from '../utils/api';
import { LineChart, Line, ResponsiveContainer, XAxis, Tooltip } from 'recharts';
import {
  IconDumbbell, IconNutrition, IconRobot, IconChart, IconUsers,
  IconClipboard, IconPhoto, IconBell, IconArrowRight, IconCheck, IconActivity, IconTarget
...
export default function Dashboard() {
  ...
  const [selectedGoal, setSelectedGoal] = useState(user?.stats?.goal || 'gain_muscle');
  ...
  const bmi = user?.stats?.height && user?.stats?.weight
    ? (user.stats.weight / ((user.stats.height / 100) ** 2)).toFixed(1)
    : null;
  ...
  const todayDone = recentWorkouts.some(w =>
    new Date(w.date || w.createdAt).toDateString() === new Date().toDateString()
  );

  const saveGoal = async () => {
    setSavingGoal(true);
    try {
      await updateUser({ stats: { ...user.stats, goal: selectedGoal } });
  ...
  const currentGoalLabel = GOALS_LIST.find(g => g.value === user?.stats?.goal)?.label || 'Not set';
      ...
      <div className="page-header">
        ...
        <div style={{ textAlign:'right' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'0.4rem', justifyContent:'flex-end', marginTop:'0.5rem' }}>
            ...
            <span style={{ fontSize:'0.83rem', fontWeight:600, color:'var(--amber-600)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.25rem', marginBottom:'1.5rem' }}>
        ...
        <div className="card animate-fade-up stagger-2">
          ...
          <ResponsiveContainer width="100%" height={130}>
            <LineChart data={weekData}>
              ...
              <Tooltip contentStyle={{ background:'var(--gray-0)', border:'1px solid var(--gray-200)', borderRadius:8, fontSize:12, boxShadow:'var(--shadow-md)' }} />
        ...
        <div className="card animate-fade-up stagger-3">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'0.625rem' }}>
            ...
            {/* Current goal with change button */}
            <div style={{ display:'flex', alignItems:'center', gap:'0.875rem', padding:'0.75rem', borderRadius:'var(--radius-lg)', border:'1.5px solid var(--gray-200)', background:'var(--gray-50)' }}>
              <div style={{ width:34, height:34, borderRadius:'var(--radius-md)', background:'#f5f3ff', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                <IconTarget size={17} color="#7c3aed" />
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:'0.73rem', color:'var(--gray-400)' }}>Current Goal</div>
                <div style={{ fontSize:'0.85rem', fontWeight:500, color:'var(--gray-900)' }}>{currentGoalLabel}</div>
              </div>
              <button className="btn btn-secondary btn-sm" onClick={() => setShowGoalModal(true)} style={{ flexShrink:0, fontSize:'0.72rem' }}>Change</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {showGoalModal && (
        <div className="modal-overlay" onClick={() => setShowGoalModal(false)}>
          <div className="modal" style={{ maxWidth:420 }} onClick={e => e.stopPropagation()}>
            ...
            <h2 style={{ fontFamily:'var(--font-display)', fontSize:'1.6rem', marginBottom:'0.5rem' }}>Change Your Goal</h2>
            <p style={{ fontSize:'0.83rem', color:'var(--gray-400)', marginBottom:'1.25rem' }}>Select your new primary fitness goal</p>
            <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem', marginBottom:'1.5rem' }}>
              {GOALS_LIST.map(g => (
                <div key={g.value} onClick={() => setSelectedGoal(g.value)}
                  style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.875rem 1rem', borderRadius:'var(--radius-lg)', cursor:'pointer', border:`1.5px solid ${selectedGoal === g.value ? 'var(--accent-primary)' : 'var(--gray-200)'}`, background:selectedGoal === g.value ? 'var(--brand-50)' : 'var(--gray-0)', transition:'all 0.15s' }}>
                  <span style={{ fontSize:'0.87rem', fontWeight:500, color:selectedGoal === g.value ? 'var(--accent-primary)' : 'var(--gray-800)' }}>{g.label}</span>
                  {selectedGoal === g.value && <IconCheck size={16} color="var(--accent-primary)" />}
                </div>
              ))}
            </div>
            <div style={{ display:'flex', gap:'0.75rem' }}>
              <button className="btn btn-secondary flex-1" onClick={() => setShowGoalModal(false)} style={{ justifyContent:'center' }}>Cancel</button>
              <button className="btn btn-primary flex-1" onClick={saveGoal} disabled={savingGoal} style={{ justifyContent:'center' }}>
