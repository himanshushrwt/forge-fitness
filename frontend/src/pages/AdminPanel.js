...
const APPROVAL_BADGE = { pending:'badge-amber', approved:'badge-green', rejected:'badge-red' };
...
export default function AdminPanel() {
  ...
  const [rejectModal, setRejectModal] = useState(null); // user being rejected
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setCoaches([
        { id:'t1', _id:'t1', name:'Marcus Steel',  email:'marcus@forge.fit',  role:'coach', coachProfile:{ isVerified:true,  rating:4.9, reviewCount:127, specializations:['Strength Training','Powerlifting'] }},
        { id:'t2', _id:'t2', name:'Sofia Ramirez', email:'sofia@forge.fit',   role:'coach', coachProfile:{ isVerified:true,  rating:4.8, reviewCount:89,  specializations:['Yoga','Functional Fitness'] }},
        { id:'t3', _id:'t3', name:'Jordan Kestrel',email:'jordan@forge.fit',  role:'coach', coachProfile:{ isVerified:true,  rating:4.7, reviewCount:63,  specializations:['Running','HIIT'] }},
        { id:'t4', _id:'t4', name:'Priya Sharma',  email:'priya@forge.fit',   role:'coach', coachProfile:{ isVerified:true,  rating:4.9, reviewCount:142, specializations:['Nutrition','Body Composition'] }},
        { id:'t5', _id:'t5', name:'Alex Thunder',  email:'alex@forge.fit',    role:'coach', coachProfile:{ isVerified:false, rating:4.6, reviewCount:51,  specializations:['Boxing','MMA Conditioning'] }},
      ]);
      setComplaints([
        { id:'c1', submitterName:'John Doe', submitterEmail:'john@example.com', againstName:'Unknown Coach', againstType:'coach', category:'fake_profile', description:'This coach has no real certifications.', status:'open', createdAt:new Date(Date.now()-86400000*2) },
        { id:'c2', submitterName:'Sarah K.', submitterEmail:'sarah@example.com', againstName:'Platform', againstType:'platform', category:'other', description:'The AI coach gave incorrect advice about my knee injury.', status:'open', createdAt:new Date(Date.now()-86400000) },
      ]);
      setPendingUsers([]);
  ...
  const removeCoach = async (coach) => {
    if (!window.confirm(`Remove ${coach.name} from the platform?`)) return;
    try {
  ...
  const TABS = [
    ...
    { key:'registrations',  label:'Registrations',  Icon:IconUsers, badge: pendingCount },
    ...
    <div className="page-container animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Approve registrations, verify coaches, manage complaints, monitor bookings</p>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display:'flex',gap:'2px',background:'var(--gray-100)',borderRadius:'var(--radius-lg)',padding:'3px',border:'1px solid var(--gray-200)',maxWidth:680,marginBottom:'1.75rem',flexWrap:'wrap' }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            style={{ flex:1,padding:'0.5rem 0.75rem',borderRadius:'var(--radius-md)',border:'none',cursor:'pointer',fontSize:'0.78rem',fontWeight:activeTab===t.key?600:500,display:'flex',alignItems:'center',justifyContent:'center',gap:'0.35rem',transition:'all 0.15s',background:activeTab===t.key?'var(--gray-0)':'transparent',color:activeTab===t.key?'var(--gray-900)':'var(--gray-500)',boxShadow:activeTab===t.key?'var(--shadow-xs)':'none',minWidth:90 }}>
        ...
        <div>
          <div className="grid-4" style={{ marginBottom:'1.75rem' }}>
            {[
              ...
              { label:'Pending Approvals',       value:stats?.pendingRegistrations||pendingCount||'0', color:'#d97706', bg:'#fffbeb', Icon:IconBell },
            ].map(s => (
          ...
          <div className="card" style={{ marginBottom:'1.5rem' }}>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'0.75rem' }}>
              {[
                { label:'Approve Registrations', desc:'Review pending accounts', color:'amber',  tab:'registrations', Icon:IconUsers },
                ...
                <button key={a.label} onClick={() => setActiveTab(a.tab)}
                  style={{ padding:'1rem',background:'var(--gray-50)',border:'1.5px solid var(--gray-200)',borderRadius:'var(--radius-lg)',cursor:'pointer',textAlign:'left',transition:'all 0.15s',position:'relative' }}
                  ...
                  <a.Icon size={20} color="var(--accent-primary)"/>
                  ...
                  {a.tab==='registrations' && pendingCount>0 && (
                    <span style={{ position:'absolute',top:8,right:8,background:'#f59e0b',color:'white',fontSize:'0.6rem',fontWeight:700,padding:'1px 6px',borderRadius:'999px' }}>
          ...
          <div className="alert alert-amber">
            ...
            <div>
              <strong>Admin Authority:</strong> All athlete and coach registrations require your approval before access is granted. You can also verify coaches, handle complaints, and monitor bookings.
            </div>
          </div>
        </div>
      )}
        ...
        <div>
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.25rem',flexWrap:'wrap',gap:'0.5rem' }}>
            <h3 style={{ fontFamily:'var(--font-display)',fontSize:'1.25rem' }}>
              ...
              <span style={{ fontSize:'0.83rem',color:'var(--gray-400)',fontFamily:'var(--font-body)',fontWeight:400,marginLeft:'0.5rem' }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:'1rem',flexWrap:'wrap' }}>
                    ...
                    <div style={{ flex:1,minWidth:160 }}>
                      <div style={{ display:'flex',alignItems:'center',gap:'0.5rem',marginBottom:'0.2rem',flexWrap:'wrap' }}>
                        ...
                        <span style={{ background: u.role==='coach'?'#f5f3ff':'#eff6ff',color:u.role==='coach'?'#7c3aed':'#2563eb',fontSize:'0.63rem',fontWeight:700,padding:'1px 7px',borderRadius:'999px',textTransform:'uppercase',letterSpacing:'0.05em' }}>
                      ...
                      <div style={{ fontSize:'0.73rem',color:'var(--gray-500)',marginTop:'0.2rem' }}>
                        ...
                        {u.coachProfile?.bio && <> · Bio: {u.coachProfile.bio.slice(0,60)}{u.coachProfile.bio.length>60?'…':''}</>}
                      </div>
                    </div>

                    {/* Actions */}
                    <div style={{ display:'flex',gap:'0.5rem',flexWrap:'wrap' }}>
                      <button className="btn btn-green btn-sm" onClick={() => approveUser(u)}
                        style={{ display:'flex',alignItems:'center',gap:'0.3rem' }}>
                        <IconCheck size={13}/> Approve
                      </button>
                      <button className="btn btn-danger btn-sm" onClick={() => { setRejectModal(u); setRejectNote(''); }}
                        style={{ display:'flex',alignItems:'center',gap:'0.3rem' }}>
          ...
          {rejectModal && (
            <div className="modal-overlay" onClick={() => setRejectModal(null)}>
              <div className="modal" style={{ maxWidth:440 }} onClick={e=>e.stopPropagation()}>
                <button className="modal-close" onClick={() => setRejectModal(null)}><IconX size={14}/></button>
                <h2 style={{ fontFamily:'var(--font-display)',fontSize:'1.5rem',marginBottom:'0.5rem' }}>Reject Registration</h2>
                <p style={{ fontSize:'0.83rem',color:'var(--gray-400)',marginBottom:'1.25rem' }}>
                  Rejecting <strong>{rejectModal.name}</strong> ({rejectModal.role})
                </p>
                <div className="form-group">
                  <label className="form-label">Reason (optional — for internal records)</label>
                ...
                <div style={{ display:'flex',gap:'0.625rem' }}>
                  ...
                  <button className="btn btn-danger flex-1" onClick={rejectUser} style={{ justifyContent:'center' }}>
                    <IconX size={13}/> Confirm Reject
                  </button>
                </div>
              </div>
            </div>
