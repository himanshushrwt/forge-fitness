...
import {
  ...
  IconUsers, IconChart, IconRobot, IconBell, IconLogout, IconSettings, IconTarget
} from './Icons';
    ...
    <div className="app-layout">
        ...
        <nav className="sidebar-nav">
          {NAV.map((item,i) => {
            if (item.section) return (
              <div key={i} className="nav-section-label" style={item.section==='Administration'?{color:'#d97706'}:{}}>{item.section}</div>
            );
            ...
            return (
              <button key={item.path} className={`nav-item ${isActive?'active':''}`}
                ...
                {item.isAdmin && <span style={{ marginLeft:'auto',fontSize:'0.6rem',background:'#fef3c7',color:'#92400e',padding:'1px 5px',borderRadius:4,fontWeight:700 }}>ADMIN</span>}
              </button>
            );
          })}
        </nav>
        ...
        <div className="sidebar-footer">
          <div style={{ display:'flex',alignItems:'center',gap:'0.625rem',marginBottom:'0.625rem' }}>
            ...
            <div style={{ flex:1,minWidth:0 }}>
              <div style={{ fontSize:'0.83rem',fontWeight:500,color:'var(--text-primary)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{user?.name}</div>
              <div style={{ fontSize:'0.7rem',color:'var(--text-muted)',textTransform:'capitalize',display:'flex',alignItems:'center',gap:'0.3rem' }}>
                {user?.role==='admin' && <span style={{ color:'#d97706',fontWeight:700 }}>Admin</span>}
      ...
      <main className="main-content">
        <div style={{ display:'none',alignItems:'center',justifyContent:'space-between',padding:'0.875rem 1.25rem',background:'var(--bg-card)',borderBottom:'1px solid var(--border-subtle)',position:'sticky',top:0,zIndex:50,boxShadow:'var(--shadow-xs)' }} className="mobile-header">
          <button onClick={()=>setSidebarOpen(true)} style={{ background:'var(--gray-100)',border:'none',borderRadius:'var(--radius-sm)',padding:'6px 8px',cursor:'pointer',color:'var(--text-primary)' }}>
          ...
          <div className="avatar" style={{ cursor:'pointer',width:30,height:30,fontSize:'0.7rem' }} onClick={()=>{ navigate('/profile'); setSidebarOpen(false); }}>{initials}</div>
        </div>
        <Outlet/>
      </main>
